<?php
@require("conn.php");
	$sql="SELECT * FROM gsat_cards_table WHERE card_package = 'PACKAGE 300' ORDER BY id desc";
	$result=$conn->query($sql);
	if($result->num_rows > 0){
		echo "<table class='employee_table'><caption>GSat Card Package 300 Monitoring</caption><tr>
	...
	$sql1 = "select sum(outgoing_quantity) as totalReleased from gsat_cards_table WHERE card_package = 'PACKAGE 300'";
	$result1 = $conn->query($sql1);
	if($result1->num_rows > 0){
		$row1 = $result1->fetch_assoc();
		$totalReleased = $row1['totalReleased'];
	}
	//THIS IS FOR GETTING THE TOTAL REMAINING BALANCE OF GSAT CARD 300
	$sql2 = "select sum(remaining_balance) as totalReceived from gsat_cards_table WHERE card_package = 'PACKAGE 300'";
	$result2 = $conn->query($sql2);
	if($result2->num_rows>0){
		$row2 = $result2->fetch_assoc();
		$totalReceived = $row2['totalReceived'];
		$balance = $totalReceived - $totalReleased;
		echo "<p class='total'>" . "Total numbers of remaining GSat 300 package card: " .  "<b class='value'>" . $balance . "</b>" . "</p>";
	}
	}
	else{
