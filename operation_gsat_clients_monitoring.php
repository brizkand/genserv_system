...
<script>
	$(document).ready(function(){
		...
		$('#searchGsatClients').load('operation_gsat_search_monitoring.php');
		$('#GSatsearch').on('keyup', function(){
			var GSatValue = $(this).val().toUpperCase();
			$.post("operation_gsat_search_monitoring.php",{searchGsatClients: GSatValue},function(result){
				$("#searchGsatClients").html(result);
			});
		});
	});
</script>
...
<div class="whole"> 
	...
	<div class = "main">
		<div class='pageTitle'><i class="fa fa-gears"></i> GSAT CLIENTS MONITORING</div>
		<!--THIS IS FOR CLIENT THAT EXPIRE WITHIN 15 DAYS-->
		<div class='box'>
		<input type="search" name="search" id='GSatsearch' placeholder="Search GSat clients here!...">
		
		<div id = 'searchGsatClients' class="box" style="overflow-x:auto;">
		</div>
		</div>
	</div>
</div>
